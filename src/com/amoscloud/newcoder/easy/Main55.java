...
public class Main55 {
    ...
    public static void main2(String[] args) {
        ...
        String starter = words.get(k);
        words.remove(starter);
        words.sort(new Comparator<String>() {
            @Override
        ...
        StringBuilder res = new StringBuilder();  // 返回值就是一个字符串
        res.append(starter);
        String first = starter;
        String snapshot = "";
        while (words.size() != 0 && first != snapshot) {   // 用快照标记是否继续循环，快照标的可以选 words 的长度，或者是否更新连接字符串。
            for (int i = 0; i < words.size(); i++) {
                snapshot = first;
                String word = words.get(i);
                if (word.startsWith(first.substring(first.length() - 1))) {
                    res.append(word);
                    first = word;
                    words.remove(word);
                    break;
