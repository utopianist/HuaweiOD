...
53,80,68,24,39,76,66,16,100,55,53,80,55
   ...
   */
    ...
    public static void main1(String[] args) {
        ...
        map.entrySet()
                .stream()
                .filter(x -> x.getValue().size() >= 3)  // 如果一个选手成绩少于3个，则认为选手的所有成绩无效，排名忽略该选手
                .sorted((o1, o2) -> {
                    Integer sum1 = sumT3(o1.getValue());
                    Integer sum2 = sumT3(o2.getValue());
                    if (sum1.equals(sum2)) {   // 如果选手的成绩之和相等，则相等的选手按照其id降序排列;3号=5号>7号>4号,输出5,3,7,4
                        return o2.getKey() - o1.getKey();  // 降序，后者减去前者
                    } else {
                        return sum2 - sum1;   //  降序，后者减前者
                    }
                })
                .map(Map.Entry::getKey)
                .forEach(x -> builder.append(x).append(","));

        System.out.println(builder.substring(0, builder.length() - 1));  // 5,3,7,4,

    }

    private static Integer sumT3(List<Integer> list) {
        list.sort(Integer::compareTo);   // 按其最高三个分数之和
        int sum = 0;
