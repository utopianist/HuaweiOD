...
abababb
    ...
    public static void main(String[] args) {
        ...
        HashMap<Character, Integer> map = new HashMap<>();  // map 统计次数
        for (char c : str.toCharArray()) {
            map.put(c, map.getOrDefault(c, 0) + 1);
        }

        print(map.entrySet().stream().filter(e -> e.getKey() >= 'a'));
        print(map.entrySet().stream().filter(e -> e.getKey() <= 'Z'));  // 且小写字母在大写字母之前
    ...
    private static void print(Stream<Map.Entry<Character, Integer>> stream) {
        List<Map.Entry<Character, Integer>> list = stream
                .sorted((o1, o2) -> {
                    ...
                    char k2 = o2.getKey();
                    if (v1 != v2) {     // 次数是否相同
                        return v2 - v1;  // 不等，降序，后者减前者
                    } else {            // 按照自然顺序排序(x:120,y:121,X:88),且小写字母在大写字母之前
                        return k1 - k2; // 相等，按照字母升序
                    }
